...
def _qt_select_multiple(items, _is_episode=False, **kwargs):
    ...
    def _show():
        ...
        for item in str_items:
            ...
            parts = item.split(", ")
            season_part = parts[0].lstrip("S") if len(parts) >= 2 else parts[0]
            display = parts[0] if len(parts) < 2 else f"S{season_part}  {parts[1]}"
            ...
            try:
                for _attempt_uhd in range(2):
                    ...
                    if w:
                        ...
                        _gui_call(lambda j=_job_handle: setattr(w, '_dl_job', j))
                        _gui_call(lambda c=ep_cwd: setattr(w, '_dl_cwd', c))
                    if w:
                        _gui_call(lambda: setattr(w, '_dl_cancelled', False))
                    ...
                    while True:
                        ...
                        while True:
                            ...
                            raw_bytes = _rawbuf[:_split]
                            _rawbuf   = _rawbuf[_split + len(sep):]
                            try:
                            ...
                            if sep in (b'\n', b'\r\n'):
                                ...
                                # Count track headers: "1 Video", "1 Audio", "2 Subtitles"
                                _tc = _re_dl.match(
                                    r'^(\d+)\s+(Video|Audio|Subtitle)', clean)
                                if _tc:
                                    _tracks_expected[0] += int(_tc.group(1))

                                # Track downloads finished line — belt-and-braces
                                ...
                                kind, status_str = _parse_r_line(raw)
                                if not kind:
                                    continue
                                if kind == 'mux_progress':
                                    continue  # ticker handles mux label
                                if kind == 'track_done':
                    ...
                    if _rawbuf:
                        try:
                            raw = _rawbuf.decode('utf-8', errors='replace')
                            clean = _ansi_re.sub('', raw).strip()
                            if 'Selection unavailable in UHD' in clean:
                                _saw_uhd_error[0] = True
...
class TwinVineLauncher(QMainWindow):
    ...
    def _build_download_page(self) -> QWidget:
        ...
        dl_panel_layout.addWidget(self._dl_status_label)
    ...
    def _do_run_batch(self):
        ...
        if episode_list:
            ...
            if hasattr(self, '_opts_slow') and self._opts_slow.isChecked():
                _slow = True
                try: _slow_min = max(1, int(self._opts_slow_min.text()))
                except ValueError: pass
                try: _slow_max = max(_slow_min, int(self._opts_slow_max.text()))
                except ValueError: pass
            elif hasattr(self, '_url_slow') and self._url_slow.isChecked():
                _slow = True
                try: _slow_min = max(1, int(self._url_slow_min.text()))
                except ValueError: pass
                try: _slow_max = max(_slow_min, int(self._url_slow_max.text()))
                except ValueError: pass

            _launch_all_powershell(episode_list, slow_mode=_slow,
                                    slow_min=_slow_min, slow_max=_slow_max)
    ...
    def _on_service_clicked(self, service_name: str):
        ...
        try:
            ...
            # Patch check_uhd if present — BBC's UHD help page now returns 404
            # which propagates as an uncaught exception and kills the download.
            # When HLG is ticked, return a match-all so AVAILABLE_HLG stays True.
            if hasattr(loader_cls, "check_uhd"):
                _orig_cuhd = loader_cls.check_uhd
                _hlg_ticked = self._hlg_cb.isChecked()
                def _make_safe(orig, hlg):
                    def _safe(self):
                        try:
                            result = orig(self)
                            if result:
                        ...
                        if hlg:
                            ...
                            return [_MatchAll("available")]
                        return []
                    return _safe
                loader_cls.check_uhd = _make_safe(_orig_cuhd, _hlg_ticked)

            # Patch BaseLoader.runsubprocess to inject _opts_extra_args —
            # BBC loader calls self.runsubprocess() directly, bypassing
            # _qt_runsubprocess where we normally inject quality flags.
            try:
                from vinefeeder import base_loader as _bl_mod
                _w = self
                _orig_rs = _bl_mod.BaseLoader.runsubprocess
                def _make_rs(orig, w):
                    def _rs(self, command):
                        if w and hasattr(w, '_opts_extra_args') and w._opts_extra_args:
                            try:
                                dl_idx = command.index('dl')
                                for arg in reversed(w._opts_extra_args):
                                ...
                                pass
                        return orig(self, command)
                    return _rs
                _bl_mod.BaseLoader.runsubprocess = _make_rs(_orig_rs, _w)
            except Exception as _e:
                self._append_log(f"[debug] runsubprocess patch skipped: {_e}")
    ...
    def _dl_update_status(self, text: str):
        if text:
            self._dl_status_label.setText(text)
            self._dl_status_label.setVisible(True)
        else:
            self._dl_status_label.setVisible(False)
    ...
    def _cleanup_temp_files(self):
        ...
        import shutil as _shutil
        cwd = getattr(self, '_dl_cwd', None)
        if not cwd:
            return
        # N_m3u8DL-RE writes segments to a Temp folder inside the TwinVine directory
        temp_dir = Path(cwd) / "Temp"
        if temp_dir.is_dir():
            try:
                # Clear contents but leave the Temp folder itself in place
