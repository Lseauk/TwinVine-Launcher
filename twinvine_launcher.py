"""
TwinVine Launcher  v1.0.0 BETA
=======================
A self-contained Windows application for TwinVine.

HOW IT WORKS
------------
Rather than launching VineFeeder as a subprocess, this launcher:

1.  Adds the TwinVine venv's site-packages to sys.path so it can import
    vinefeeder and its service modules directly.

2.  Monkey-patches base_loader.BaseLoader's beaupy-based selection methods
    (display_series_list, display_episode_list, display_final_episode_list,
    display_beaupylist, list_display_beaupylist) to open native Qt dialogs
    instead of taking over the terminal.  The service __init__.py files never
    need to know the difference — they just call self.display_series_list() etc.
    and get back the same data types as before.

3.  Monkey-patches the console.input() calls in base_loader (series number
    selection) with a Qt input dialog.

4.  Captures subprocess output from runsubprocess() into the in-app log panel.

5.  The launcher window itself is styled like VineFeeder — dark Catppuccin
    Mocha palette, service buttons, search box, batch mode slider —
    so the user experience is identical, just all in one window.

REQUIREMENTS (build machine)
-----------------------------
pip install PyQt6 requests pyinstaller

The target machine only needs the TwinVine installation produced by the
Install tab (git + uv sync).  The launcher adds the venv to sys.path at
runtime — no separate Python install needed on the user's machine.
"""
...
from PyQt6.QtWidgets import (
    ...
    QLabel, QLineEdit, QPushButton, QFrame, QCheckBox, QSlider,
...
def _qt_select_multiple(items, **kwargs):
...
def _qt_runsubprocess(self, command):
    ...
    We write a small .ps1 helper script to a temp file and tell PowerShell
    to execute it.  This completely sidesteps the wt.exe / semicolon /
    quoting nightmare — wt.exe only gets ONE argument (the script path)
    so it cannot misinterpret semicolons as tab separators.
    """
...
def patch_base_loader():
    ...
    def _display_episode_list(self, series_name):
        ...
        selected_strs = _qt_select_multiple(episode_list)
        ...
        try:
            ...
            for toml_path in list(d.rglob("pyproject.toml")):
                try:
                    ...
                    if "<=3.13" in text:
                        patched = _re.sub(
                            r'requires-python\s*=\s*">=3\.10,\s*<=3\.13"',
                            'requires-python = ">=3.10, <=3.99"',
                            text
                        )
                        if patched != text:
            ...
            for l in _run_cmd([uv_exe, "sync", "--python", sys_py], cwd=d): self._log(l)
            ...
            if dst.exists():
                try:
                    ...
                    self._log(f"Patched vaults path to: {vaults_posix}")
...
class TwinVineLauncher(QMainWindow):
    ...
    def _build_download_page(self) -> QWidget:
        ...
        self._hlg_cb.setToolTip(
            "Request HLG (High Dynamic Range) streams.\n"
            "Uncheck if download fails with \'Selection unavailable in UHD\'.")
        self._hlg_cb.setStyleSheet(
            f"color:{C['subtext']};font-size:11px;padding:0 8px;")
        ...
        self._dl_proc = None  # current download subprocess
    ...
    def _on_service_clicked(self, service_name: str):
        ...
        if text:
            if "http" in text:
                inx, text_to_pass, found = 1, text, None
            else:
        ...
        if "Browse" not in action:
            ...
            def _go():
                ...
                if "Greedy" in action:
                    ...
                    self._launch_service(1, val, None)
                else:
    ...
    def _on_update_result(self, remote: str, local: str):
        ...
        if not remote:
            ...
            return
        if remote == local:
            QMessageBox.information(self, APP_NAME, "✓ TwinVine is up to date!")
        else:
            ans = QMessageBox.question(self, APP_NAME,
                "A new version is available!\n\nUpdate now? (git pull + uv sync)")
...
1. On first run, click **Install / Update** in the sidebar and then **Install TwinVine Tools**.
   This downloads around 700MB of tools and may take 10-15 minutes.
2. Once installed, return to the **Home** and click a service button (BBC, ITVX, etc.).
3. Choose how to search — by keyword, URL, or browse by category.
4. Select the series and episodes you want, then confirm to begin downloading.


## Navigation

- **Home** — The main page. Click a service button to start, type keywords to search,
  or paste a direct episode URL into the search box.
...
- **Envied Config** — Opens envied.yaml in Notepad. Edit credentials, download location,
  filename format, subtitle settings and more.
- **HLG** — Enables HDR/HLG streams when ticked (default on). Restart the app and untick if you see a
  "Selection unavailable in UHD" error.
...
- **Download by URL** — Paste a direct episode URL to download it immediately.
- **Browse by Category** — Browse the service's categories to find content.

After searching, select the series you want, then tick the episodes and click Confirm.
Multiple episodes download sequentially — you can mix episodes from different series.
Progress is shown in the download panel with a live log and cancel option.
...
2. Search and select episodes as normal — they queue instead of downloading immediately.
   The sidebar shows how many episodes are queued as soon as you start adding episodes.
3. When ready, click **Run Batch** to download everything in the queue at once.
4. Remember to toggle off Batch Mode to resume normal downloads. 


## Common Errors

**"Selection unavailable in UHD"** — Restart the app then untick the HLG checkbox on the Download page and try again.

**"No .venv found"** — Go to Install / Update and click Install TwinVine Tools.

**"patch failed / cannot import vinefeeder"** — Usually a Python version mismatch.
Delete the TwinVine subfolder and click Install TwinVine Tools again.

**"Download fails with exit code 1"** — Check your credentials in Envied Config.
Make sure your CDM device file is in the WVDs folder.
...
ALL4, BBC iPlayer, ITVX, MY5, PLEX, RTE, STV, TPTV, TVNZ(Untested by me), U
...
- If a fresh install fails, delete the TwinVine subfolder and try again.
  Do not delete envied.yaml as it holds your credentials and settings.
    ...
    def _build_about_page(self) -> QWidget:
        ...
        title = QLabel("TwinVine Launcher")
        title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        title.setStyleSheet(
            f"font-size:22px;font-weight:bold;color:{C['green']};")
        c.addWidget(title)

        ver = QLabel(f"v{APP_VERSION}")
        ver.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ver.setStyleSheet(f"color:{C['subtext']};")
        c.addWidget(ver)
        ...
        info = QLabel(
            "TwinVine (VineFeeder + Envied) by vinefeeder / A_n_g_e_l_a\n\n"
            "This launcher imports VineFeeder's service modules directly,\n"
            "replacing beaupy terminal prompts with native Qt selection dialogs.\n"
            "Everything runs in one window — no PowerShell required."
        )
        info.setAlignment(Qt.AlignmentFlag.AlignCenter)
        info.setStyleSheet(f"color:{C['subtext']};line-height:1.6;")
        c.addWidget(info)
        ...
        c.addWidget(gh_btn, alignment=Qt.AlignmentFlag.AlignCenter)
